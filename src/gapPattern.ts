 ...
 */
export function* gapPattern(max: number, gaps: [number, number]): Generator<number> {
    let pos = 0;
    let i = 0;
    while (pos <= max) {
        yield pos;
 ...
 */
export function bandSpans(max: number, gaps: [number, number]): Array<[number, number]> {
 ...
 */
export function nextGapTick(threshold: number, gaps: [number, number]): number {
    let pos = 0;
    let i = 0;
    while (pos < threshold) {
        pos += gaps[i % 2];
